...
// The dev-identity list stays available regardless of whether Firebase is
// configured — `run.py` opens several of these at once via `?dev=<subject>`
// to test multiple family members side by side, which several real Google
// accounts can't do as conveniently.
const DEV_IDENTITIES = [
  ...
  { subject: "sharma-senior", label: "Vikram Sharma — cared-for person" },
  { subject: "iyer-owner", label: "Meera Iyer — a different family" },
  { subject: "iyer-caregiver", label: "Arjun Iyer — caregiver, Iyer family" },
  { subject: "iyer-senior", label: "Lakshmi Iyer — cared-for person, Iyer family" },
  { subject: "dual-caregiver", label: "Priya Rao — belongs to both families" },
];
  ...
  return (
    ...
    >
      ...
      <form className="space-y-4" onSubmit={submitDev}>
        ...
        <Button type="submit" variant={firebaseEnabled ? "outline" : "default"} className="w-full h-12 font-medium" disabled={loading}>
          {loading ? (
            <>
              <Loader2 className="w-4 h-4 mr-2 animate-spin" />
              Signing in…
