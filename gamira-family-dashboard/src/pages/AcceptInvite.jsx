...
export default function AcceptInvite() {
  ...
  useEffect(() => {
    ...
    acceptInvitationOnce(token)
      .then(async () => {
        // Refresh the session so the new family and, if this invite links a
        // cared-for person, the updated senior all show up immediately.
  ...
  if (status === "accepting") {
    return (
      <div className="fixed inset-0 flex flex-col items-center justify-center gap-3">
        ...
        <p className="text-[13px] text-muted-foreground">Joining the family…</p>
      </div>
    );
  }
  ...
  return (
    <AuthLayout
      icon={CheckCircle2}
      title="You're in"
      subtitle="You now have access to this family's care record."
    >
      <Button className="w-full h-12 font-medium" onClick={() => navigate("/")}>
        Continue
      </Button>
